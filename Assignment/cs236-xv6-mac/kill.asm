...
main(int argc, char **argv)
{
  ...
  4b:	6a 02                	push   $0x2
  4d:	e8 f6 02 00 00       	call   348 <printf>
    exit();
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 2cd:	85 c9                	test   %ecx,%ecx
 2cf:	74 04                	je     2d5 <printint+0x19>
 2d1:	85 d2                	test   %edx,%edx
 2d3:	78 6b                	js     340 <printint+0x84>
    neg = 1;
    x = -xx;
  } else {
    x = xx;
 2d5:	89 55 c4             	mov    %edx,-0x3c(%ebp)
  neg = 0;
 ...
 2e1:	8d 75 d7             	lea    -0x29(%ebp),%esi
  do{
    buf[i++] = digits[x % base];
 2e4:	8b 45 c4             	mov    -0x3c(%ebp),%eax
 2e7:	31 d2                	xor    %edx,%edx
 2e9:	f7 f3                	div    %ebx
 2eb:	89 cf                	mov    %ecx,%edi
 2ed:	8d 49 01             	lea    0x1(%ecx),%ecx
 2f0:	8a 92 9c 06 00 00    	mov    0x69c(%edx),%dl
 2f6:	88 54 3e 01          	mov    %dl,0x1(%esi,%edi,1)
  }while((x /= base) != 0);
 2fa:	8b 55 c4             	mov    -0x3c(%ebp),%edx
 2fd:	89 45 c4             	mov    %eax,-0x3c(%ebp)
 300:	39 da                	cmp    %ebx,%edx
 302:	73 e0                	jae    2e4 <printint+0x28>
  if(neg)
 304:	8b 55 08             	mov    0x8(%ebp),%edx
 307:	85 d2                	test   %edx,%edx
 309:	74 07                	je     312 <printint+0x56>
    buf[i++] = '-';
 30b:	c6 44 0d d8 2d       	movb   $0x2d,-0x28(%ebp,%ecx,1)
 310:	89 cf                	mov    %ecx,%edi

  while(--i >= 0)
 312:	8d 5d d8             	lea    -0x28(%ebp),%ebx
 315:	8d 7c 3d d8          	lea    -0x28(%ebp,%edi,1),%edi
 319:	8d 76 00             	lea    0x0(%esi),%esi
    putc(fd, buf[i]);
 31c:	8a 07                	mov    (%edi),%al
 31e:	88 45 d7             	mov    %al,-0x29(%ebp)
  write(fd, &c, 1);
 321:	50                   	push   %eax
 322:	6a 01                	push   $0x1
 324:	56                   	push   %esi
 325:	ff 75 c0             	push   -0x40(%ebp)
 328:	e8 0e ff ff ff       	call   23b <write>
  while(--i >= 0)
 32d:	89 f8                	mov    %edi,%eax
 32f:	4f                   	dec    %edi
 330:	83 c4 10             	add    $0x10,%esp
 333:	39 d8                	cmp    %ebx,%eax
 335:	75 e5                	jne    31c <printint+0x60>
}
 ...
 33f:	90                   	nop
    x = -xx;
 340:	f7 da                	neg    %edx
 342:	89 55 c4             	mov    %edx,-0x3c(%ebp)
 345:	eb 98                	jmp    2df <printint+0x23>
 347:	90                   	nop
...
printf(int fd, const char *fmt, ...)
{
 ...
 35b:	74 2a                	je     387 <printf+0x3f>
 35d:	43                   	inc    %ebx
  ap = (uint*)(void*)&fmt + 1;
 35e:	8d 4d 10             	lea    0x10(%ebp),%ecx
 361:	89 4d d4             	mov    %ecx,-0x2c(%ebp)
    c = fmt[i] & 0xff;
 364:	0f b6 d0             	movzbl %al,%edx
    if(state == 0){
      if(c == '%'){
 367:	83 fa 25             	cmp    $0x25,%edx
 36a:	74 24                	je     390 <printf+0x48>
        state = '%';
      } else {
        putc(fd, c);
 ...
 36f:	50                   	push   %eax
 370:	6a 01                	push   $0x1
 372:	8d 45 e7             	lea    -0x19(%ebp),%eax
 375:	50                   	push   %eax
 376:	56                   	push   %esi
 377:	e8 bf fe ff ff       	call   23b <write>
  for(i = 0; fmt[i]; i++){
 37c:	43                   	inc    %ebx
 37d:	8a 43 ff             	mov    -0x1(%ebx),%al
 380:	83 c4 10             	add    $0x10,%esp
 383:	84 c0                	test   %al,%al
 385:	75 dd                	jne    364 <printf+0x1c>
        putc(fd, c);
      }
      state = 0;
    }
 ...
 38f:	90                   	nop
  for(i = 0; fmt[i]; i++){
 390:	8a 13                	mov    (%ebx),%dl
 392:	84 d2                	test   %dl,%dl
 394:	74 f1                	je     387 <printf+0x3f>
    c = fmt[i] & 0xff;
 396:	0f b6 c2             	movzbl %dl,%eax
      if(c == 'd'){
 399:	80 fa 25             	cmp    $0x25,%dl
 39c:	0f 84 fe 00 00 00    	je     4a0 <printf+0x158>
 3a2:	83 e8 63             	sub    $0x63,%eax
 3a5:	83 f8 15             	cmp    $0x15,%eax
 3a8:	77 0a                	ja     3b4 <printf+0x6c>
 3aa:	ff 24 85 44 06 00 00 	jmp    *0x644(,%eax,4)
 3b1:	8d 76 00             	lea    0x0(%esi),%esi
 3b4:	88 55 d0             	mov    %dl,-0x30(%ebp)
        putc(fd, '%');
 ...
 3c1:	57                   	push   %edi
 3c2:	56                   	push   %esi
 3c3:	e8 73 fe ff ff       	call   23b <write>
        putc(fd, c);
 3c8:	8a 55 d0             	mov    -0x30(%ebp),%dl
 ...
 3d3:	57                   	push   %edi
 3d4:	56                   	push   %esi
 3d5:	e8 61 fe ff ff       	call   23b <write>
  for(i = 0; fmt[i]; i++){
 3da:	83 c3 02             	add    $0x2,%ebx
 3dd:	8a 43 ff             	mov    -0x1(%ebx),%al
 3e0:	83 c4 10             	add    $0x10,%esp
 3e3:	84 c0                	test   %al,%al
 3e5:	0f 85 79 ff ff ff    	jne    364 <printf+0x1c>
}
 ...
 3f3:	90                   	nop
        printint(fd, *ap, 16, 0);
 3f4:	8b 7d d4             	mov    -0x2c(%ebp),%edi
 3f7:	8b 17                	mov    (%edi),%edx
 3f9:	83 ec 0c             	sub    $0xc,%esp
 3fc:	6a 00                	push   $0x0
 3fe:	b9 10 00 00 00       	mov    $0x10,%ecx
 403:	89 f0                	mov    %esi,%eax
 405:	e8 b2 fe ff ff       	call   2bc <printint>
        ap++;
 40a:	83 c7 04             	add    $0x4,%edi
 40d:	89 7d d4             	mov    %edi,-0x2c(%ebp)
  for(i = 0; fmt[i]; i++){
 410:	eb c8                	jmp    3da <printf+0x92>
 412:	66 90                	xchg   %ax,%ax
        s = (char*)*ap;
 414:	8b 4d d4             	mov    -0x2c(%ebp),%ecx
 417:	8b 01                	mov    (%ecx),%eax
        ap++;
 419:	83 c1 04             	add    $0x4,%ecx
 41c:	89 4d d4             	mov    %ecx,-0x2c(%ebp)
        if(s == 0)
 41f:	85 c0                	test   %eax,%eax
 421:	0f 84 89 00 00 00    	je     4b0 <printf+0x168>
        while(*s != 0){
 427:	8a 10                	mov    (%eax),%dl
 429:	84 d2                	test   %dl,%dl
 42b:	74 29                	je     456 <printf+0x10e>
 42d:	89 c7                	mov    %eax,%edi
 42f:	88 d0                	mov    %dl,%al
 431:	8d 4d e7             	lea    -0x19(%ebp),%ecx
 434:	89 5d d0             	mov    %ebx,-0x30(%ebp)
 437:	89 fb                	mov    %edi,%ebx
 439:	89 cf                	mov    %ecx,%edi
 43b:	90                   	nop
          putc(fd, *s);
 43c:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 43f:	50                   	push   %eax
 440:	6a 01                	push   $0x1
 442:	57                   	push   %edi
 443:	56                   	push   %esi
 444:	e8 f2 fd ff ff       	call   23b <write>
          s++;
 449:	43                   	inc    %ebx
        while(*s != 0){
 44a:	8a 03                	mov    (%ebx),%al
 44c:	83 c4 10             	add    $0x10,%esp
 44f:	84 c0                	test   %al,%al
 451:	75 e9                	jne    43c <printf+0xf4>
 453:	8b 5d d0             	mov    -0x30(%ebp),%ebx
  for(i = 0; fmt[i]; i++){
 456:	83 c3 02             	add    $0x2,%ebx
 459:	8a 43 ff             	mov    -0x1(%ebx),%al
 45c:	84 c0                	test   %al,%al
 45e:	0f 85 00 ff ff ff    	jne    364 <printf+0x1c>
 464:	e9 1e ff ff ff       	jmp    387 <printf+0x3f>
 ...
 48e:	57                   	push   %edi
 48f:	56                   	push   %esi
 490:	e8 a6 fd ff ff       	call   23b <write>
        ap++;
 495:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
 499:	e9 3c ff ff ff       	jmp    3da <printf+0x92>
 ...
 4a3:	52                   	push   %edx
 4a4:	6a 01                	push   $0x1
 4a6:	8d 7d e7             	lea    -0x19(%ebp),%edi
 4a9:	e9 25 ff ff ff       	jmp    3d3 <printf+0x8b>
 4ae:	66 90                	xchg   %ax,%ax
          s = "(null)";
 4b0:	bf 3c 06 00 00       	mov    $0x63c,%edi
 4b5:	b0 28                	mov    $0x28,%al
 4b7:	e9 75 ff ff ff       	jmp    431 <printf+0xe9>
...
free(void *ap)
{
 ...
 4c8:	a1 b0 06 00 00       	mov    0x6b0,%eax
 ...
 4f7:	89 0a                	mov    %ecx,(%edx)
    ...
    p->s.ptr = bp;
  freep = p;
 4f9:	89 15 b0 06 00 00    	mov    %edx,0x6b0
}
 4ff:	5b                   	pop    %ebx
 500:	5e                   	pop    %esi
 501:	5f                   	pop    %edi
 ...
 536:	89 0a                	mov    %ecx,(%edx)
  freep = p;
 538:	89 15 b0 06 00 00    	mov    %edx,0x6b0
}
 53e:	5b                   	pop    %ebx
 53f:	5e                   	pop    %esi
 540:	5f                   	pop    %edi
...
malloc(uint nbytes)
{
 ...
 556:	47                   	inc    %edi
  if((prevp = freep) == 0){
 557:	8b 15 b0 06 00 00    	mov    0x6b0,%edx
 ...
 58d:	73 41                	jae    5d0 <malloc+0x8c>
    ...
    if(p == freep)
 58f:	39 05 b0 06 00 00    	cmp    %eax,0x6b0
 595:	75 ed                	jne    584 <malloc+0x40>
  p = sbrk(nu * sizeof(Header));   // sbrk(n) asks the kernel for n more bytes.
 597:	83 ec 0c             	sub    $0xc,%esp
 59a:	56                   	push   %esi
 59b:	e8 03 fd ff ff       	call   2a3 <sbrk>
  if(p == (char*)-1)
 ...
 5b1:	50                   	push   %eax
 5b2:	e8 05 ff ff ff       	call   4bc <free>
  return freep;
 5b7:	8b 15 b0 06 00 00    	mov    0x6b0,%edx
 ...
 5dc:	89 78 04             	mov    %edi,0x4(%eax)
      freep = prevp;
 5df:	89 15 b0 06 00 00    	mov    %edx,0x6b0
      return (void*)(p + 1);
 ...
 5f7:	90                   	nop
    base.s.ptr = freep = prevp = &base;
 5f8:	c7 05 b0 06 00 00 b4 	movl   $0x6b4,0x6b0
 5ff:	06 00 00 
 602:	c7 05 b4 06 00 00 b4 	movl   $0x6b4,0x6b4
 609:	06 00 00 
    base.s.size = 0;
 60c:	c7 05 b8 06 00 00 00 	movl   $0x0,0x6b8
 613:	00 00 00 
 616:	b8 b4 06 00 00       	mov    $0x6b4,%eax
